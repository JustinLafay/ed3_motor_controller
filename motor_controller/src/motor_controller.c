...
int main(void) {
	...
	while (1) {									// Loop principal
		...
		if (flags & (1 << MODO_ADC_UART)){					// Si está en modo UART
			...
			velocidad_motor = (dma_value>>4)&0xFFF;	// Actualizo la velocidad con lo recibido por DMA
		}
		...
		if (!(flags & (1 << EMERGENCIA))){				// Si la parada de emergecia no está activada
			...
			else {								// Si el valor de velocidad es mayor a 300
				flags &= ~(1 << FRENANDO);				// Deshabilito flag de motor frenado
				if (flags & 1) {				// Si el sentido de giro es 1
					LPC_GPIO2->FIOCLR |= (1 << 7);				// Prendo verde
					LPC_GPIO2->FIOSET |= (1 << 6) & (1 << 8);	// Apago rojo y azul
					LPC_PWM1->MR4 = velocidad_motor;					// Nuevo valor de PWM1.4
					LPC_PWM1->LER |= (1 << 4);					// Actualizo el valor de PWM1.4
				}

				else {							// Si el sentido de giro es 0
					LPC_GPIO2->FIOCLR |= (1 << 8);				// Prendo azul
					LPC_GPIO2->FIOSET |= (1 << 6) & (1 << 7);	// Apago rojo y verde
					LPC_PWM1->MR3 = velocidad_motor;					// Nuevo valor de PWM1.3
					LPC_PWM1->LER |= (1 << 3);					// Actualizo el valor de PWM1.3
				}
			}
		} else {
